...
const authState = async (
    sessionWhatsapp: IWhatsappSession
    ...
    const saveState = async () => {
        try {
            ...
            await WhatsappSession.updateOne({
                _id: sessionWhatsapp._id
            }, {
                credentials: JSON.stringify({ creds, keys }, BufferJSON.replacer, 0)
            })
    ...
    if (sessionWhatsapp.credentials !== null) {
        const result = JSON.parse(sessionWhatsapp.credentials, BufferJSON.reviver);
        creds = result.creds;
        keys = result.keys;
    } else {
        creds = initAuthCreds();
